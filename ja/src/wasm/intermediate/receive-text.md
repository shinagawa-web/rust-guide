...
ただし、いまうまくいくのは、正しい JSON がきちんと送られてきたときだけです。本文が JSON になっていなかったり、`a` や `b` が入っていなかったり、そもそも POST ではなく GET で叩かれたりすると、いまのハンドラはエラーを返すだけで、何が悪かったのかは呼ぶ側に伝わりません。次のページでは、こうした失敗に対して、理由の分かる返し方をしていきます。
