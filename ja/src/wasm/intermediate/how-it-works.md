...
前のページではサンプルを手元で動かしました。`wrangler dev` で起動して `curl` で応答が返ることを確かめましたがいくつか棚上げにしたことがあります。Rust を書いているのになぜ npm で入れた wrangler が要るのかなぜ JavaScript のファイルが出てくるのかそもそも Rust のコードがどうやってエッジで動くのか。このページでその仕組みを順に見ていきます。

このページの道すじが一度わかればこの先のページはその上に積んでいくだけになります。

## ビルドで何が作られるのか

さきほど `wrangler dev` を動かしたときその裏で `build` というフォルダができています。clone した直後には無かったもので`wrangler dev` が起動時に worker-build を呼び出して自動で作る生成物なのでリポジトリには含まれていません。
...
この3つがリクエストを処理するときにどう並ぶのかを見るとそれぞれの役目がはっきりします。さきほど `curl` を送ったときその1回のリクエストは次の順にこれらを通っていきました。
...
リクエストはまず Cloudflare のランタイムに届き入口の shim.mjs から index.js へ進んでその先の wasm にたどり着きwasm が出した結果は来た道を逆にたどってレスポンスとして返ります。さきほど `textdiff worker is running` が返ってきたのはこの道を一往復した結果でした。
...
前のページで近くで動けるのは Cloudflare が世界中に拠点を構えているからと書きましたがその拠点網の正体が CDN です。
...
エッジコンピュートはその同じ拠点網の上であなたのコードも動かせるようにしたものでCloudflare ではこうして動かすプログラムを Worker と呼びます。手元で動かしてきたのがその Worker で新しいネットワークを引くのではなくCDN が築いた拠点をそのまま計算の場所として使います。
...
これが前のページで見た「近い＝速い」の正体です。CDN が世界中に築いた拠点がそのまま計算の場所になっているので差分の計算を遠くの1台まで往復させず利用者の最寄りの拠点でそのまま済ませられます。

## まとめ

ここまでで手元で動いた1回のリクエストが裏で何を通っていたのかを見てきました。Rust は wasm に変換され JavaScript に包まれて V8 アイソレートの中で動きます。そのアイソレートは Cloudflare が CDN として世界中に広げた拠点の上に載っています。はじめに棚上げにした「なぜ wrangler が要るのか」「なぜ JavaScript が出てくるのか」「どうやってエッジで動くのか」はこのページで順に見てきました。

ここまでが土台です。次のページではリクエストを受け取る `fetch` ハンドラに初級で手を入れたコードを差し込みいまの決まった文字列のかわりに差分を計算して返すようにします。ここから先は手を動かして機能を足していきます。
