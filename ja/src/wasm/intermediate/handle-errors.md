...
どちらも `500 Internal Server Error` としか返りません。`500` は本来、サーバー側で何か問題が起きたことを表す番号です。だから呼ぶ側からは、自分の送り方が違うのか、本文が JSON になっていないのか、それともサーバーが本当に落ちているのかまで、見分けがつきません。呼ぶ側のミスなのに、サーバーの障害と同じ番号で返っているわけです。前のページで、読み取りに失敗したら `?` でそのまま中断する、と書いたところが、この中身の分からないエラーになって出ています。
...
POST でないので、`405` が返りました。前は中身の分からないエラーでしたが、今度は送り方が違うと番号で分かります。`with_cors` で付けた `Access-Control-` の3つのヘッダも、ちゃんと乗っています。次に、JSON として読み取れない本文を POST してみます。
...
残るは、これを自分の手元から、世界のどこからでも呼び出せる公開の URL に載せることです。次のページで、いよいよデプロイして公開します。
