...
前のページで、送られてきたテキストの差分を JSON で返せるようになりました。ただし、うまくいくのは正しい JSON が POST で送られてきたときだけです。それ以外の呼び方をすると、いまのハンドラが何を返すのか、まず手元で見てみます。

`wrangler dev` を起動しておき、本文のない GET で呼び出してみます。状態コードも見たいので、`curl` に `-i` を付けます。
...
次に、JSON として読み取れない本文を POST してみます。ここでは `not json` という、JSON になっていないただの文字列を送ります。
...
どちらも `500 Internal Server Error` としか返りません。`500` は本来、サーバー側で何か問題が起きたことを表す番号です。だから呼ぶ側からは、自分の送り方が違うのか、本文が JSON になっていないのか、それともサーバーが本当に落ちているのかまで、見分けがつきません。呼ぶ側のミスなのに、サーバーの障害と同じ番号で返っているわけです。前のページで、読み取りに失敗したら `?` でそのまま中断する、と書いたところが、この理由の分からないエラーになって出ています。

これでは、呼ぶ側は何を直せばいいのか分かりません。GET をやめて POST にすればいいのか、送った本文を直せばいいのか、`500` という番号からは読み取れないからです。呼ぶ側が手を打てるようにするには、何が問題だったのかを、こちらから伝えてやる必要があります。

このページでは、そうした失敗に理由をつけて、呼ぶ側が自分で直せる形で応えるようにします。あわせて、ブラウザの中の HTML から呼び出せるようにもします。ここまで整えば、公開して誰かに呼び出されても、きちんと受け答えできる HTTP エンドポイントになります。
...
末尾の `?` は、読み取りに失敗したらそこで中断してエラーを返す印でした。ただこのままだと、返るのは理由の分からないエラーで、呼ぶ側には何が悪かったのかが伝わりません。ここを、失敗を自分で受け止めて、`400` を返すように書き換えます。
...
エラーを返す行に付いた `?` は、これまでとは少し違う使い方です。ここまで `?` は、失敗したらそこで中断する印として使ってきました。じつは `?` には、成功した値を取り出す、もう一つの働きがあります。`Response::error(...)` が返すのは、レスポンスそのものではなく、それを `Result` で包んだものです。`?` でその包みからレスポンスを取り出し、`with_cors` に渡しています。この行で失敗が起きるわけではなく、`Result` に包まれた中身を使うために取り出しているだけです。

## 手元で動かして確かめる

書き換えたら、手元で動かして、失敗のときの応えを確かめます。さきほどと同じように、`wrangler dev` を起動しておき、まず GET で呼び出してみます。
...
POST でないので、`405` が返りました。前は理由の分からないエラーでしたが、今度は送り方が違うと番号で分かります。次に、JSON として読み取れない本文を POST してみます。
...
JSON として読み取れなかったので、`400` が返りました。正しく呼んだときは、これまでどおり差分が返ります。
...
正しく呼ばれたときは差分を返し、間違った呼び方には理由の分かる番号を返せるようになりました。手元で動く HTTP エンドポイントとして、ひととおり整いました。さきほど付けた許可のヘッダも各レスポンスに乗っていますが、これが効くのはブラウザから呼んだときです。

残るは、これを自分の手元から、世界のどこからでも呼び出せる公開の URL に載せることです。次のページで、いよいよデプロイして公開し、ブラウザからも呼んで、その許可のヘッダが効くところまで確かめます。
