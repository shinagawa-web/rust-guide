...
前のページまでで、差分ツールは手元で動く HTTP エンドポイントとして整いました。ただし動いているのは自分のパソコンの中だけで、呼び出せるのも同じパソコンからに限られます。このページでは、Cloudflare のエッジに載せて、世界のどこからでも呼び出せる公開の URL にします。

`wrangler dev` で動かしてきたのは、本物のエッジと同じランタイムを手元で立ち上げたものでした。手元にあるだけなので、外からは見えません。今度は同じコードを Cloudflare へ送り、世界中の拠点に配ってもらいます。送るのに使うのも、やはり wrangler です。

## Cloudflare のアカウントを用意する

エッジにコードを送るには、Cloudflare のアカウントが要ります。まだ持っていなければ、[Cloudflare のサインアップページ](https://dash.cloudflare.com/sign-up) から作れます。学習でこの差分ツールを動かすくらいなら、[無料の範囲](https://developers.cloudflare.com/workers/platform/pricing/)（1日あたり10万リクエスト）で足ります。公開したものは、動きを確かめたら取り下げられます。このページの最後で、その手順もやります。
...
出力に何度か出てくる `textdiff-worker` は、この Worker の名前です。サンプルの `wrangler.toml` に `name = "textdiff-worker"` と書いてあり、wrangler はその名前でエッジに公開します。次の行の URL の先頭にも、同じ名前がそのまま使われています。
...
さっそく、公開された URL にアクセスしてみます。これまで `http://localhost:8787` に送っていた curl の宛先を、いま控えた自分の URL に変えるだけです。
...
公開した URL が curl からもブラウザからも呼べることを確かめられました。ひととおり確認できたら、公開したものは取り下げておくのがおすすめです。自分で見ていない場所に、動いたままのものを残しておく必要はありません。取り下げは、サンプルのフォルダで実行します。

```sh
$ wrangler delete
```

これで、公開した Worker が Cloudflare から消え、その URL も使えなくなります。このまま使い続けたいなら、取り下げずに残しておいてもかまいません。

## ここまでで作ったもの

初級で手を入れた差分の計算を、同じ Rust のまま Cloudflare のエッジに載せ、誰でも呼び出せる URL として公開しました。JSON でテキストを受け取り、差分を JSON で返し、間違った呼び方には理由の分かる番号で応え、ブラウザからの呼び出しにも備える。手元で動かして作ってきたものが、そのまま世界のどこからでも呼び出せるようになりました。
