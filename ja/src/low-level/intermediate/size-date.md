...
`ls -al` と比べると、size（284）は右に揃えられておらず、mtime（1720598720）は読める日付になっていません。この章ではその2つを整えます。
...
`meta.mtime()` が返す `i64` は、Unix エポック（1970年1月1日00:00:00 UTC）からの経過秒数です。生の秒数を人が読める形にするには、日付と時刻への変換が要ります。
