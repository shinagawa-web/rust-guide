...
Rust の文法を網羅的に学ぶのではなく、バグを一つ修正する過程で必要な知識だけを順番に積み上げます。ゼロからツールを作れるようになるのが理想ですが、そこまでの分量を用意すると最後まで辿り着けません。既存のコードを読んで直す、というスタイルにすることで、最短で「動くものを変えた」体験を積めるようにしています。このガイドを終えると、ファイルのパーミッションがどんなビットでできているかを理解しながらバグを修正し、何をどう変えたかを自分の言葉で説明できるようになります。
...
全 9 ページ・約 20,000 字。コードを手元で動かしながら読み進めると、約 2 時間で完走できます。

前半はコードリーディングを通して、パーミッションの 10 文字が 1 個の整数からどう組み立てられているかを理解します。コードの理解が進むと、どこを直せばいいかが自然と掴めてきます。後半では実際に直して動作を確認し、テストで確かめてから、変更を自分の言葉でまとめます。

以下の順で進みます。

1. [プロジェクトを動かす](beginner/cargo.md) ― 環境を整えて、サンプルを動かす
2. [Rust の基本構文](beginner/basics.md) ― サンプルに出てくる書き方だけ先に確認する
3. [ビットとマスクの基本](beginner/bits.md) ― パーミッションの整数を読むためのビット演算を押さえる
4. [ファイルのメタデータを読む](beginner/read-metadata.md) ― OS から `mode` を受け取る部分を読む
5. [main.rs を読む](beginner/read-main.md) ― 全体がどうつながっているかを読む
6. [perm.rs を読む](beginner/read-perm.md) ― 10 文字を組み立てる中心の処理を読む
7. [file_type を書き直す](beginner/fix-file-type.md) ― 種別のビットを見て先頭の 1 文字を直す
8. [テストを追加する](beginner/add-test.md) ― ディレクトリのテストを足す
9. [変更をふり返る](beginner/review-change.md) ― 何をどう変えたかを自分の言葉でまとめる
...
- `if / else` の条件分岐
- ビットのマスク（`&`）で種別を判定する
- `assert_eq!` でテストを 1 つ足す

読めれば十分なこと

- `fn`（引数と戻り値の型）
- `u32` / `char` / `String` などの型
- シフト（`>>`）とマスク（`& 0b111`）でビットを取り出す式
- `match` でエラーを分岐する読み方
- `mod` / `use` / `&`（コードを辿れる程度）
