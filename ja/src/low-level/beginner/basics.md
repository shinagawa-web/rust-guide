...
> 📝 ここでは「`&` は中身を借りて読むための記号」と押さえれば十分です。その裏側にある所有権という仕組みを理解していなくても、初級は問題なく進められます。「所有する」と「参照して見るだけ（借りる）」の違いが気になったら「[所有権](../../concepts/ownership.md)」を読んでみてください。
...
これで、サンプルコードに出てくる構文はひととおり押さえました。ただし `perm.rs` には、`mode >> 6` や `& 0b111` のように、パーミッションの整数（`mode`）のビットを直接操作する書き方も出てきます。次のページでは、その土台になるビットとマスクの考え方を見ていきます。
