...
そもそも、なぜ貸してまで書き換えるのでしょうか。持ち主のところで書き換えられるなら、`&mut` は要りません。要るのは、書き換えたい処理が別の関数に分かれているときです。関数に切り出すと、値の持ち主（呼び出し元）と、書き換える人（その関数）が別々になります。このとき、所有権ごと渡すと呼び出し元は値を手放してしまいますし、読むだけの `&` では書き換えられません。所有権は渡さずに、書き換えだけを任せる。これが `&mut` です。
