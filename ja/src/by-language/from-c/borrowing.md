...
C の配列は、関数に渡した瞬間に先頭要素へのポインタに崩れます。配列そのものではなく、先頭のアドレスだけが写るので、いくつ要素があるかという情報は、渡した先には届きません。

```c
// C
void print_all(int xs[]) {
    // ここで sizeof(xs) は配列全体ではなく、ポインタのサイズ（8 など）を返す
    // 要素数は、この関数の中からは分からない
}

int main(void) {
    int xs[3] = {1, 2, 3};
    print_all(xs); // 渡るのは &xs[0] だけ。長さ 3 は一緒に来ない
}
```
...
## ポインタと長さを一つにまとめたのが、スライス

Rust は、この「先頭ポインタ」と「長さ」を一つの値にまとめます。それがスライスで、型は `&[T]` と書きます。`&` が付いているとおり、これは借用の一種です。配列や `Vec` の全体、あるいは一部分を、所有権を渡さずに貸します。貸す先へ、先頭の位置と長さがいつも一緒に届きます。

さきほどの関数は、Rust ではこう書けます。長さの引数は要りません。

```rust
// Rust
fn print_all(xs: &[i32]) { // スライスは長さを内側に持っている
    for x in xs {
        println!("{x}");
    }
}
...
fn main() {
    let xs = [1, 2, 3];
    print_all(&xs); // 配列全体を貸す。長さ 3 も一緒に渡る
}
```

`&xs` で渡るのは、先頭ポインタと長さ 3 の組です。`print_all` は `xs.len()` で長さを取れますし、`for x in xs` は、その長さの分だけ回って自分で止まります。C のように長さを別途渡す必要も、渡した長さと配列がずれる心配もありません。
