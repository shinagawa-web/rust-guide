...
個数を省略できるようにして、空欄なら 1 個として扱う、と決めているとします。そういう既定値を用意する場面では、`Option` のときと同じ `unwrap_or` が使えます。
...
`NaN` が黙って後ろへ流れていたのと違い、こちらは「空欄なら 1 個で進める」と決めたことがコードに残ります。既定値で進めてよいか、いくつにするかは、書いた側がそのつど選ぶことです。
