...
もう一つの違いは初期化です。C++ では初期化していない変数を読むと未定義動作でした。Rust では `let x: i32;` のように初期値なしで宣言することは問題ありませんが、初期化する前に読もうとするとコンパイルエラーになります。
...
C++ は `static_cast` なしで整数型を暗黙に変換できる場面がありました。Rust は数値型が違えば暗黙には変換しません。変換したいなら `as` を書きます。
...
C++ では符号付き整数のあふれは未定義動作、符号なしは静かに巻き戻りでした。Rust はデバッグビルドで実行時にパニックして気づかせます。リリースビルドでは C++ の符号なし整数と同様に巻き戻ります。
...
呼び出し方はドット記法の点で同じです。C++ では `p.add(q)` と書き、Rust では `p.add(&q)` と参照を明示します。`self` に付いた `&` が所有権と借用の話で、次の章の本題になります。
