...
Rust には GC がありません。その代わりに、値をいつ解放するかを、実行時ではなくコンパイル時に決めます。それを担うのが所有権という仕組みです。ここは、Go から来た人が最初に戸惑い、そして理解に時間がかかるところです。逆に言えば、ここが腑に落ちれば、このあとのエラー処理や並行性で出てくる `&` や `move` も、同じ理屈の続きとして読めるようになります。
