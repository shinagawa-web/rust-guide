...
無いときは既定値で済ませたいなら、`unwrap_or` が使えます。Go でゼロ値をそのまま既定値に使っていたのを、明示的に書く形です。
...
`unwrap()` や `expect()` もあり、これは中身を取り出しますが、`None` のときは panic します。「ここは絶対にあるはず」と言い切れる場面に限る最終手段です。Go で `nil` を確かめずに触って panic するのに近いですが、Rust では `unwrap` という名前がコードに残るので、どこで確認を飛ばしたかが読めば分かります。
