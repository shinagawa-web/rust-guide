...
まず、パソコン側の道具をそろえます。ふだんの Rust プログラムは、書いてそのまま自分のパソコンで動きます。組み込みでは、書いた Rust を micro:bit のマイコン向けにコンパイルし、USB 経由でボードに送り込む、というひと手間が要ります。まず Rust 本体を入れ、それから組み込み用の道具を足していきます。
...
1つ目は、micro:bit のマイコン向けにコンパイルするための target です。ふだん `cargo build` すると、いま使っているパソコンの CPU 向けの機械語ができます。micro:bit の CPU はそれとは別の種類（ARM Cortex-M）なので、そのままでは動きません。その CPU 向けに翻訳できるよう、コンパイラに target を1つ足します。

```sh
$ rustup target add thumbv7em-none-eabihf
```

`thumbv7em-none-eabihf` が micro:bit のチップ（nRF52833）の CPU の種類を表す名前です。覚える必要はありません。真ん中の `none` は「OS が無い」という意味で、OS の無いマイコンに向けてコンパイルする、という話がこの名前にそのまま出ています。
