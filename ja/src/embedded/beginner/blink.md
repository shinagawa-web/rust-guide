...
まず、パソコン側の道具をそろえます。ふつうの Rust プログラムは、書いてそのまま自分のパソコンで動きます。組み込みでは、書いた Rust を micro:bit のマイコン向けにコンパイルし、USB 経由でボードに送り込む、というひと手間が要ります。まず Rust 本体を入れ、それから組み込み用の道具を足していきます。
