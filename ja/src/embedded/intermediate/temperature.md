...
この章を終えると、nRF Connect で micro:bit に接続したとき、気温の値が1秒ごとに届くようになります。

2章のコードでは「接続が来たら INFO ログを出して終わる」だけでした。ここでは接続中にチップ内蔵の温度センサを読み続け、Notify でスマホへ送る仕組みを加えます。
...
サーバー構造体は `main` の前に書きます。2章の `main.rs` に次の2ブロックを追加してください（`static ADV_DATA` の宣言より前であれば構いません）。
...
`main` 内では `let _conn` を `let conn` に変えます。2章では接続情報を使わないため `_conn` と書いていましたが、3章では Notify の送信先として `&conn` を渡すために名前が必要になります。
...
4章で `Server` に LED サービスを追加すると、`gatt_server::run` のクロージャに `ServerEvent::Led(...)` アームを足す必要があります。

---

次の章では、スマホから micro:bit の LED を操作します。
