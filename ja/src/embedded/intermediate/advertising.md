...
Nordic Semiconductor の開発者サイトで「nRF5 SDK 17.1.0」を検索してダウンロードします。zip を展開すると `components/softdevice/s140/hex/` の中に次のファイルがあります。
...
```sh
probe-rs download --chip nRF52833_xxAA <path>/s140_nrf52_7.3.0_softdevice.hex
```
...
[dependencies]
embassy-executor = { version = "0.7", features = ["task-arena-size-32768", "arch-cortex-m", "executor-thread", "defmt"] }
embassy-nrf      = { version = "0.3", features = ["defmt", "nrf52833", "time-driver-rtc1", "gpiote"] }
embassy-time     = { version = "0.4", features = ["defmt"] }
embassy-futures  = { version = "0.1" }
nrf-softdevice   = { version = "0.1", features = ["nrf52833", "s140", "ble-peripheral", "ble-gatt-server"] }
...
`nrf-softdevice` の `s140` と `ble-peripheral` が BLE に必要なフィーチャです。`ble-gatt-server` は3章以降で使います。
...
`DEFMT_LOG = "debug"` を設定しないと RTT ログが出力されません。切り分けに必要なので最初から入れておきます。
