...
最後の `files.sort()` は、名前順に並べるための一行です。`read_dir` が返す順番は環境によって変わるので、並べ替えておくと、いつ実行しても同じ順で表示されます。並べ替えるのはこのディレクトリの中だけです。ソート済みの一覧を `targets` の末尾に足していくので、ディレクトリを複数渡したときの順番や、直接渡したファイルの順番は、指定したとおりに保たれます。
