...
`#[arg(short = 'l', long)]` は、このフィールドを `-l`（短い形）と `--lines`（長い形）の両方で指定できるようにする宣言です。型を `bool` にしているので、指定されれば `true`、指定されなければ `false` になります。短い形の文字は、行数・文字数・バイト数を数える定番ツール `wc` に合わせて、行を `-l`、文字を `-m`、バイトを `-c` にしています。
